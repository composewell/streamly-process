...
parseManySingleQuote = P.many FL.mconcat parseSingleQuoteArg
...
parseManyDoubleQuote = P.many FL.mconcat parseDoubleQuoteArg
...
escapeArgument arg =
    case S.parse parseArgument (S.fromList arg) of
        Right str -> str
...
escapeCommand command = case splitOnSpace command of
    [] -> error "No Executable"
    (execFilePath : unEscapedArgList) ->
        (execFilePath, map escapeArgument unEscapedArgList)